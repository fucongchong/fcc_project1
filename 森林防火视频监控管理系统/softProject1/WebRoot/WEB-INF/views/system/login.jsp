	...
	$(function(){
		...
		$("#submitBtn").click(function(){
			...
			$.ajax({
				...
				success: function(data){
					if("success" == data.type&&"工作人员"==data.Utype){					
						window.parent.location.href = "index";
					}
					else if("success" == data.type&&"管理员"==data.Utype)
					{
					   window.parent.location.href = "index";
					}
					else if("success" == data.type&&"专家"==data.Utype)
					{
					  window.parent.location.href = "index";
					}			
					else{
		...
		$("#registerBtn").click(function(){
			...
			$.ajax({
				...
				dataType: "json", //返回数据类型
				success: function(data){
					if("goRegister" == data.type){					
						window.parent.location.href = "goRegister";
					}else{
						$.messager.alert("消息提醒", data.msg, "warning");
						$("#vcodeImg").click();//切换验证码
						$("input[name='vcode']").val("");//清空验证码输入框
...
<title>登录|森林防火视频监控管理系统</title>
<meta name="keywords" content="森林防火视频监控管理系统">
</head>
<body>

<div class="header" style="padding: 0;">
	<h2 style="color: white; width: 400px; height: 60px; line-height: 60px; margin: 0 0 0 30px; padding: 0;">森林防火视频监控管理系统</h2>
</div>
    ...
    <form id="form" class="form form-horizontal" method="post">
      <div class="row cl">
        ...
        <div class="formControls col-8">
          <input id="UID" name="UID" value="" type="text" placeholder="账号" class="input-text size-L">
        </div>
      </div>
      <div class="row cl">
        <label class="form-label col-3"><i class="Hui-iconfont">&#xe60e;</i></label>
...
<div class="footer">计算机17-1班 邓卓宏 付聪冲&nbsp; 陈佳丽 尚凌云 尹朦朦 </div>
